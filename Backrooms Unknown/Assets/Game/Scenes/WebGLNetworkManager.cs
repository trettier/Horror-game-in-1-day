...
public class WebGLNetworkManager : NetworkManager
{
    // Overrides the base singleton so we don't
    // have to cast to this type everywhere.
    public static new WebGLNetworkManager singleton => (WebGLNetworkManager)NetworkManager.singleton;
    ...
    public override void Start()
    {
        ...
        try
        {
            ...
            if (swt != null)
            {
                Debug.Log($"[Server] ===>>> Listening on port: {swt.port}");
                Debug.Log($"[Server] ===>>> WebSocket URL: ws://{GetLocalIPAddress()}:{swt.port}/");
            }
            else
            {
                Debug.LogWarning("[Server] SimpleWebTransport not found.");
            }
        }
        catch (Exception ex)
        {
    ...
    public static string GetLocalIPAddress()
    {
        ...
        {
            Debug.LogWarning($"[Server] Failed to get local IP: {ex.Message}");
        }
    ...
    public override void OnStartClient() {
        ...
        try
        {
            ...
            Debug.Log($"[Client] Transport: {transport.GetType().Name}");

            if (transport is Mirror.SimpleWeb.SimpleWebTransport swt)
            {
                Debug.Log("[Client] Using SimpleWebTransport");

                // networkAddress задается в NetworkManager (обычно IP или домен)
                string protocol = swt.clientUseWss ? "wss" : "ws";
                string host = NetworkManager.singleton.networkAddress;
                int port = swt.port;

                Debug.Log($"[Client] Connecting to {protocol}://{host}:{port}");
            }
        }
        catch (Exception ex)
        {
